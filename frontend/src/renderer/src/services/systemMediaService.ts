...
export function pickNowPlayingSession(sessions: SystemMediaSession[]): SystemMediaSession | null {
...
function getElectronAPI(): any {
  ...
  return win.electronAPI ?? win.api ?? null
}
...
export function subscribeMediaSessions(callback: (sessions: SystemMediaSession[]) => void): () => void {
  const api = getElectronAPI()
...
export function getMediaControlTarget(session: SystemMediaSession | null | undefined): MediaControlTarget | undefined {
...
export function getAppIconName(appName: string): {
  ...
  if (lower.includes('spotify')) return { vendor: 'material', name: 'spotify', color: '#1DB954', bg: '#1d1d1dff' }
  if (lower.includes('youtube')) return { vendor: 'material', name: 'youtube', color: '#fff', bg: '#FF0000' }
  if (lower.includes('firefox')) return { vendor: 'material', name: 'firefox', color: '#fff', bg: '#FF7139' }
  if (lower.includes('chrome')) return { vendor: 'material', name: 'google-chrome', color: '#fff', bg: '#4285F4' }
  if (lower.includes('edge')) return { vendor: 'material', name: 'microsoft-edge', color: '#fff', bg: '#0078D7' }
  if (lower.includes('groove') || lower.includes('media player')) return { vendor: 'ionicons', name: 'musical-notes', color: '#000', bg: '#fff' }
